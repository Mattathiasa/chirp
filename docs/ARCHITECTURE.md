...
* File transfer resumes after an interruption and is verified end to end, but it is not streamed: the sender reads the whole file into memory and the receiver holds the finished bytes in memory to hash and encrypt them. A 100 MB transfer therefore costs about 100 MB of RAM on each side. Chunking the hash and encryption would fix it and is not done.
